...
urlpatterns = [
    ...
    path('districts/towns/<int:pk>', towns_view, name='towns'),  # вывод всех внесенных городов в данную область страны
    # path('towns/<int:pk>', TownsView.as_view()),
    path('districts/towns/town_watch/<int:pk>', town_watch_view),  # вывод конкретного города по запросу (что посмотреть)
    path('districts/towns/town_eat/<int:pk>', town_eat_view),  # вывод конкретного города по запросу (где поесть)
    path('districts/towns/town_sleep/<int:pk>', town_sleep_view),  # вывод конкретного города по запросу (где поспать)
